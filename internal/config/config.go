...
type Export struct {
	...
	ProgressEvery int64         `mapstructure:"progress_every"`
}
...
type ProviderClient struct {
	PSK                string        `mapstructure:"psk"`
	AllowedDNSSuffixes []string      `mapstructure:"allowed_dns_suffixes"`
...
func setDefaults(v *viper.Viper) {
	v.SetDefault("app.name", "data-export-service")
	...
	v.SetDefault("database.schema", "data_export")
	...
	v.SetDefault("jwt.issuer", "data-export-service")
	...
	v.SetDefault("auth.audience", "data-export-service")
	...
	v.SetDefault("migration.table", "data_export_schema_migrations")
	...
	v.SetDefault("event_bus.consumer_ack_wait", "2h5m")
	...
	v.SetDefault("export.batch_size", 500)
	v.SetDefault("export.max_rows", 1000000)
	v.SetDefault("export.max_bytes", 1<<30)
	v.SetDefault("export.job_timeout", "2h")
	v.SetDefault("export.result_ttl", "168h")
	v.SetDefault("export.worker_count", 2)
	v.SetDefault("export.progress_every", 5000)
	...
	v.SetDefault("object_storage.bucket", "platform-exports")
	...
	v.SetDefault("provider_client.psk", "")
	v.SetDefault("provider_client.allowed_dns_suffixes", []string{".platform.svc.cluster.local"})
	...
	v.SetDefault("provider_client.retry.methods", []string{"/platform.export.v1.ExportProviderService/DescribeDataset"})
	...
	v.SetDefault("provider_client.tls.enabled", false)
...
func (c Config) Validate() error {
	...
	if c.EventBus.Enabled && (len(c.EventBus.URLs) == 0 || c.EventBus.StreamName == "" || len(c.EventBus.Subjects) != 1 || c.EventBus.Subjects[0] != "platform.>" || (c.EventBus.Storage != "file" && c.EventBus.Storage != "memory") || c.EventBus.MaxAge <= 0 || c.EventBus.DuplicateWindow <= 0 || c.EventBus.ConnectTimeout <= 0 || c.EventBus.ReconnectWait <= 0 || c.EventBus.PublishTimeout <= 0 || c.EventBus.ConsumerAckWait <= 0 || c.EventBus.ConsumerMaxDeliver <= 0 || c.EventBus.DispatchInterval <= 0 || c.EventBus.DispatchBatchSize <= 0 || c.EventBus.DispatchLease <= 0 || c.EventBus.DispatchRetryDelay <= 0 || c.EventBus.PublishedRetention < c.EventBus.MaxAge || c.EventBus.CleanupInterval <= 0 || c.EventBus.CleanupBatchSize <= 0) {
		return errors.New("enabled event_bus requires URLs, stream, canonical platform.> subjects, valid storage, positive delivery/dispatch/cleanup settings, and published retention at least max_age")
	}
	...
	if c.Cron.Enabled && c.Cron.MetadataCleanupSpec != "" && (c.Cron.CleanupBatchSize <= 0 || c.Cron.MetadataRetention <= 0) {
	...
	if c.ServiceRegistry.Enabled {
		if c.ServiceRegistry.Target == "" || len(c.ServiceRegistry.PSK) < 32 || c.ServiceRegistry.MaxStale <= 0 || c.ServiceRegistry.SnapshotDirectory == "" || len(c.ProviderClient.PSK) < 32 || len(c.ProviderClient.AllowedDNSSuffixes) == 0 || c.ProviderClient.Timeout <= 0 || c.ProviderClient.FailureCooldown <= 0 {
			return errors.New("enabled service_registry requires registry and provider credentials, cache, DNS allowlist, and positive timeouts")
		}
		if c.App.Env == "production" && (!c.ServiceRegistry.TLS.Enabled || !c.ProviderClient.TLS.Enabled || c.ServiceRegistry.AllowInsecure || c.ProviderClient.AllowInsecure) {
			return errors.New("production service discovery and provider calls require TLS")
		}
		if (!c.ServiceRegistry.TLS.Enabled && !c.ServiceRegistry.AllowInsecure) || (!c.ProviderClient.TLS.Enabled && !c.ProviderClient.AllowInsecure) {
			return errors.New("plaintext registry or provider credentials require explicit allow_insecure")
		}
	}
