...
import (
	...
	"github.com/lihongjie0209/data-export-service/internal/idempotency"
...
func TestIdempotencyExecutionCompletesAndReplaysExportCreate(t *testing.T) {
	t.Parallel()
	...
	router.Use(RequestID(), func(c *gin.Context) {
		...
		c.Next()
	}, IdempotencyExecution(manager, []string{"/api/v1/exports/create"}, slog.New(slog.NewTextHandler(io.Discard, nil))))
	router.POST("/api/v1/exports/create", func(c *gin.Context) { calls++; OK(c, gin.H{"job_id": "job-1"}) })
	out := httptest.NewRecorder()
	router.ServeHTTP(out, httptest.NewRequest(http.MethodPost, "/api/v1/exports/create", strings.NewReader(`{"idempotency_key":"business-key"}`)))
	...
	request := httptest.NewRequest(http.MethodPost, "/api/v1/exports/create", strings.NewReader(`{"idempotency_key":"business-key"}`))
...
func TestIdempotencyExecutionBypassesExportReadsAndDownload(t *testing.T) {
	t.Parallel()
	for _, route := range []string{"/api/v1/exports/datasets/list", "/api/v1/exports/datasets/describe", "/api/v1/exports/get", "/api/v1/exports/list", "/api/v1/exports/download"} {
...
func TestExportHTTPRequirementCoversEveryBusinessRoute(t *testing.T) {
	t.Parallel()
	for _, route := range []string{"/api/v1/exports/datasets/list", "/api/v1/exports/datasets/describe", "/api/v1/exports/create", "/api/v1/exports/get", "/api/v1/exports/list", "/api/v1/exports/cancel", "/api/v1/exports/retry", "/api/v1/exports/download"} {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			router.POST("/api/v1/external/callback", func(c *gin.Context) {
				...
				if test.status == http.StatusOK && (!ok || value.ID != "data-export-service:psk" || value.Type != platformprincipal.TypeServiceAccount) {
