...
import (
	...
	"github.com/lihongjie0209/data-export-service/internal/environment"
	platformexport "github.com/lihongjie0209/data-export-service/internal/export"
	apphealth "github.com/lihongjie0209/data-export-service/internal/health"
	...
	"github.com/lihongjie0209/data-export-service/internal/requestid"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, authService *auth.Service, authorizer platformauthz.Authorizer, healthService *apphealth.Service, exportService *platformexport.Service, catalog *platformexport.Catalog, idempotencyManager *idempotency.Manager, metrics *observability.Metrics, logger *slog.Logger) (*Server, error) {
	options := []grpc.ServerOption{
		grpc.MaxRecvMsgSize(cfg.GRPC.MaxReceiveBytes),
		grpc.StatsHandler(otelgrpc.NewServerHandler()),
		grpc.ChainUnaryInterceptor(environmentInterceptor(cfg.Runtime.ActiveProfile), requestIDInterceptor, idempotencyInterceptor, recoveryInterceptor(logger), authInterceptor(authService, cfg.Auth), platformauthz.UnaryServerInterceptor(authorizer, exportGRPCRequirement(cfg.Authorization.Enabled)), platformidempotency.UnaryServerInterceptor(idempotencyManager, cfg.Idempotency.GRPCMethods, logger), metricsInterceptor(metrics, logger)),
...
func exportGRPCRequirement(enabled bool) platformauthz.GRPCResolver {
...
func authenticateGRPC(ctx context.Context, method string, service *auth.Service, cfg config.Auth) (context.Context, error) {
	...
	if cfg.PSK.Enabled && auth.MatchesAny(method, cfg.PSK.GRPCMethods) {
		...
		return platformprincipal.WithContext(ctx, platformprincipal.Principal{ID: "data-export-service:psk", Type: platformprincipal.TypeServiceAccount}), nil
	}
