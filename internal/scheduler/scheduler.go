...
func New(lc fx.Lifecycle, cfg config.Config, locker *cache.Locker, worker *platformexport.Worker, metrics *observability.Metrics, logger *slog.Logger) (*cron.Cron, error) {
	...
	if cfg.Cron.ExportCleanupSpec != "" {
		if _, err := runner.AddFunc(cfg.Cron.ExportCleanupSpec, func() {
			started := time.Now()
			status := "success"
			...
			if runErr != nil {
				status = "error"
				logger.ErrorContext(ctx, "expired export cleanup failed", "error", runErr, "cleaned", cleaned)
			} else {
				logger.InfoContext(ctx, "expired export cleanup completed", "cleaned", cleaned)
			}
			metrics.ObserveCron("export_result_cleanup", status, started)
		}); err != nil {
			return nil, err
		}
	}
