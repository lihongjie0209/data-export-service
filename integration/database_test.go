...
import (
	...
	appdb "github.com/lihongjie0209/data-export-service/internal/database"
...
func TestRepositoryAndMigrations(t *testing.T) {
	for _, databaseType := range []string{"postgres", "mysql"} {
		t.Run(databaseType, func(t *testing.T) {
			...
			if databaseType == "postgres" {
				if err := db.GetContext(ctx, &exportTables, `SELECT count(*) FROM pg_tables WHERE schemaname = current_schema() AND tablename = 'export_jobs'`); err != nil {
					t.Fatal(err)
				}
				var timezone string
			...
			} else if err := db.GetContext(ctx, &exportTables, `SELECT count(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'export_jobs'`); err != nil {
				t.Fatal(err)
			}
			if exportTables != 1 {
