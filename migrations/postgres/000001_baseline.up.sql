CREATE TABLE export_jobs (
    ...
    format text NOT NULL, filename text NOT NULL, query_json text NOT NULL DEFAULT '{}',
    selected_columns_json text NOT NULL DEFAULT '[]', idempotency_key text NOT NULL, status text NOT NULL,
    rows_exported bigint NOT NULL DEFAULT 0, bytes_written bigint NOT NULL DEFAULT 0,
    progress_percent integer NOT NULL DEFAULT 0, object_key text NOT NULL, content_type text NOT NULL DEFAULT '',
    checksum text NOT NULL DEFAULT '', error_code text NOT NULL DEFAULT '', error_message text NOT NULL DEFAULT '',
    started_at timestamptz, completed_at timestamptz, expires_at timestamptz, version bigint NOT NULL DEFAULT 1,
...
CREATE INDEX export_jobs_queued_idx ON export_jobs (created_at, id) WHERE status = 'queued';
CREATE INDEX export_jobs_expiry_idx ON export_jobs (expires_at) WHERE status = 'succeeded';
...
CREATE INDEX export_outbox_pending_idx ON export_outbox_events (available_at, created_at) WHERE published_at IS NULL;
